...
Zum migrieren kann das SQL-Script auf der Datenbank ausgeführt werden mit `cat <script>.sql | docker compose exec -T db psql -U backendusr -d shareshopdb`, oder `docker compose exec db psql -U backendusr -d shareshopdb`, um eine Shell zu starten. Dabei muss das cwd auch `src/` sein.