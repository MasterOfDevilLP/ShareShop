    ...
    async login() {
      ...
      try {
        ...
        if (data.success) {
          ...
          this.errorMessage = this.language === 'de' ? 'Falscher Benutzername oder Passwort.' : 'Incorrect username or password.';
        }
      } catch (err) {
        this.errorMessage = this.language === 'de' ? 'Fehler beim Einloggen.' : 'Error logging in.';
      }
    }
  }
};
